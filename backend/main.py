...
async def process_source(request: UrlRequest):
    ...
    try:
        ...
        kb_message = {
            "id": str(uuid.uuid4()),
            "role": "knowledge_base",
            "content": f"I've processed the content from {request.url}. The document \"{document['title']}\" has been added to my knowledge base with {len(document['chunks'])} chunks of information.",
            # "timestamp": datetime.now().isoformat(),
